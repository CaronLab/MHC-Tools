...
class MixMhcPredHelper(BaseHelper):
    ...
    def _format_class_I_alleles(self, alleles: List[str]):
        ...
        return [a.replace('HLA-', '').replace(':', '').replace('H-2-', 'H2-') for a in std_alleles]
    ...
    def predict_df(self):
        ...
        if self.mhc_class == 'I':
            ...
            df_columns = ['Peptide', 'Allele', 'Core', 'EL_Rank', 'Binder']
        data = []
        for i in range(len(result_df)):
            for j in range(len(result_df.columns)):
                ...
                if self.mhc_class == 'I':
                    ...
                    data.append([peptide, allele, result_df.loc[i, 'Core_best'], el_rank, binder])
