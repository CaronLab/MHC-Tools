...
class BigMhcHelper(BaseHelper):
    def __init__(self,
                 peptides: list[str],
                 alleles: list[str]):
        super().__init__('BigMHC')
        ...
        self.output_path = Path(__file__).parent.parent / 'output' / 'bigmhc_output.csv'
    ...
    def predict_df(self, im=False, gpu=False):
        ...
        if not im:
            command = f'python {self.bigmhc_exe_path} -i {pep_file_path} -o {self.output_path} -m el -d {device}'
        else:
            command = f'python {self.bigmhc_exe_path} -i {pep_file_path} -o {self.output_path} -m im -d {device}'
