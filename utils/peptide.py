...
def filter_peptides_by_length(peptides: List[str], min_length: int, max_length: int) -> List[str]:
    """Filters peptides by length and prints status message."""
    initial_count = len(peptides)
    filtered_peptides = [p for p in peptides if min_length <= len(p) <= max_length]
    filtered_peptides = list(set(filtered_peptides))
    filtered_count = len(filtered_peptides)

    if filtered_count != initial_count:
        print(f'Peptides length out of [{min_length}, {max_length}] are filtered out. '
              f'{filtered_count} peptides left. {initial_count - filtered_count} peptides are ignored.')
    else:
        print(f'Predicting on {initial_count} peptides.')
